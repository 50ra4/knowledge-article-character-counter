...
const parseNumber = (s: string): Either.Either<Error, number> => {
  ...
  if (isNaN(i) || i % 1 !== 0) {
    return Either.left(
      new Error("You should set an integer in the articleNumber option.")
    );
  }
...
const setViewPort = F.pipe(
  partial(setViewPortToPage, DEFAULT_VIEW_PORT, IS_HEADLESS),
  IO.of
);

const isM1Mac = process.arch === 'arm64';

const launch = () =>
  launchBrowser({ headless: IS_HEADLESS, slowMo: SLOW_MOTION_MS, executablePath: !isM1Mac ? undefined : EXECUTABLE_PATH || undefined, });
const shutdown: (
  browser: Browser
) => <T>(result: Either.Either<Error, T>) => TaskEither.TaskEither<Error, T> =
  (browser) => (result) =>
    F.pipe(
...
const goToArticlePage = (numberOfArticle: number) =>
  partial(goToUrl, `${VIEW_PAGE_URL}/${numberOfArticle}`);
/**
 * go to draft article page
 */
const goToDraftArticlePage = (numberOfArticle: number) =>
  partial(goToUrl, `${DRAFT_VIEW_PAGE_URL}/${numberOfArticle}`);

/**
 * login
 */
const login = () =>
  partial(loginKnowledge, { id: LOGIN_ID, password: LOGIN_PASSWORD });
    ...
    TaskEither.chain((browser) =>
      F.pipe(
        ...
        TaskEither.chain((contents) =>
          TaskEither.of({ numberOfArticle, contents })
        ),
...
const main = F.pipe(
  ...
  parseArticleNumber(),
  TaskEither.chain(
    getIsDraft() ? getDraftArticleContents() : getArticleContents()
  ),
  TaskEither.chain(({ contents, numberOfArticle }) => {
    const length = countArticleCharacters(contents);
    return TaskEither.of(
      `#${numberOfArticle} Article has ${length} characters.`
    );
  }),
